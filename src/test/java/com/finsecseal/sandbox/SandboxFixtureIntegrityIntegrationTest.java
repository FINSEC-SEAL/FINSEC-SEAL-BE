...
@Testcontainers
@SpringBootTest
class SandboxFixtureIntegrityIntegrationTest {
    ...
    @Test
    void integrityDetectsFieldsThatWerePreviouslyOutsideRepresentativeChecks() {
        ...
        jdbcTemplate.update("""
                update sandbox_customers
                   set profile_json = jsonb_set(profile_json, '{accountNumber}', '"TAMPERED"'::jsonb)
        ...
        jdbcTemplate.update("""
                update sandbox_customers
                   set classification_json = jsonb_set(classification_json, '{syntheticOnly}', 'false'::jsonb)
                 where namespace_id = ? and customer_key = 'CUST-1003'
                """, runId);
        assertThat(fixtureService.verifyIntegrity(runId)).isFalse();
    }
    ...
    private UUID seedQueuedRun() {
        ...
        jdbcTemplate.update("""
                ...
                values (?, ?, ?, 'Fixture Integrity Agent', 'Sandbox integrity test', 'ACTIVE')
                """, agentId, WORKSPACE_ID, "fixture-integrity-agent-" + suffix);
        jdbcTemplate.update("""
        ...
        jdbcTemplate.update("""
                ...
                """, suiteId, WORKSPACE_ID, "fixture-integrity-suite-" + suffix, HASH_B);
