...
@Service
public class Fa05ExecutionOrchestrator {
    ...
    public Fa05ExecutionOrchestrator(
            ...
            HighImpactToolLoopOracleEvaluator highImpactOracleEvaluator,
        ...
        this.highImpactOracleEvaluator = highImpactOracleEvaluator;
    ...
    public Result execute(
        ...
        if (!"FA-05".equals(target.category())) {
            throw new BusinessException(
                    ...
                    "TestCase is not FA-05"
            );
        }

        AttackSeed seed =
                attackSeedCatalog.requireSeed(target.category());
        AttackVariant variant =
                attackVariantFactory.fromSeed(seed);
        validateVariantAgainstTestCase(variant, target);

        if (target.status() == TestRunStatus.COMPLETED) {
            return requireCompletedReplayResult(
                    runId,
                    testCaseId,
                    variant.variantHash()
            );
        ...
        if (target.status() != TestRunStatus.QUEUED
                && target.status() != TestRunStatus.RUNNING) {
            throw new BusinessException(
                    ErrorCode.INVALID_STATE_TRANSITION,
                    "FA-05 execution requires a QUEUED, RUNNING, or completed replayable TestRun"
            );
        }
        ...
        try {
            ...
            RunExecutionLifecycleService.CaseExecutionClaim claim =
                    lifecycleService.claimCase(
                            runId,
                            testCaseId,
                            0,
                            variant.variantHash(),
            ...
            TestRunPersistenceDto.CaseRun caseRun =
                    claim.caseRun();
            ...
            if (!claim.created()) {
                throw new BusinessException(
                        ...
                        "The same TestCase execution already exists"
                );
            }
            ...
            /*
             * FA-05 trust boundary:
             * capture the canonical state and fixture integrity BEFORE the
             * intentionally dangerous mutation runs. Post-mutation golden
             * integrity is expected to differ when the attack succeeds.
             */
            ...
            boolean integrityValid =
                    fixtureService.verifyIntegrity(runId);
            if (!integrityValid) {
                throw new BusinessException(
                        ErrorCode.EVIDENCE_INCOMPLETE,
                        "Golden Fixture integrity validation failed before FA-05 mutation"
                );
            }

            AgentToolLoopService.LoopResult loop =
                    agentToolLoopService.execute(
                            context,
                            variant,
                            normalizedActor
                    );
            ...
            HighImpactToolLoopOracleEvaluator.Evaluation
                    oracleEvaluation =
                    highImpactOracleEvaluator.evaluate(
                            context,
                            beforeSnapshot,
                            loop.toolSteps(),
                            integrityValid
            ...
            AgentToolLoopService.ToolStep oracleStep =
                    oracleEvaluation.sourceStep();
            OracleResult oracleResult =
                    oracleEvaluation.oracleResult();
            ExecutionEventDto.Event sourceEvent =
                    oracleEvaluation.sourceEvent();

            OracleAssessmentService.Assessment assessment =
                    oracleAssessmentService.record(
                            runId,
                            caseRun.id(),
                            traceId,
                            sourceEvent.eventId(),
            ...
            TestCaseRunStatus terminalCaseStatus =
                    caseStatus(oracleResult.outcome());
            ...
            caseResult.put(
                    "oracleResultId",
                    assessment.oracleResult().id().toString()
            );
            caseResult.put(
                    "variantHash",
                    variant.variantHash()
            ...
            if (oracleStep.delivery() != null
                    && oracleStep.delivery().status() != null) {
                caseResult.put(
                        "deliveryStatus",
                        oracleStep.delivery().status().name()
                );
            }
            if (oracleStep.delivery() != null
                    && oracleStep.delivery()
            ...
            if (assessment.finding() != null) {
                caseResult.put(
                        "findingId",
                        assessment.finding().id().toString()
                );
            }
    ...
    private Result requireCompletedReplayResult(
            UUID runId,
            UUID testCaseId,
            String variantHash
    ) {
        List<UUID> caseRunIds = jdbcTemplate.query(
                """
                select id
                  from test_case_runs
                 where test_run_id = ?
        ...
        if (caseRunIds.size() != 1) {
            throw new BusinessException(
                    ErrorCode.EVIDENCE_INCOMPLETE,
                    "Completed FA-05 TestRun is missing its unique terminal TestCaseRun"
            );
        }

        TestRunPersistenceDto.CaseRun caseRun =
                runPersistenceService.findCase(
                        caseRunIds.getFirst()
                );
        ...
        if (!isReplayableTerminal(caseRun.status())) {
            throw new BusinessException(
                    ErrorCode.EVIDENCE_INCOMPLETE,
                    "Completed FA-05 TestRun contains a non-terminal TestCaseRun"
            );
        }
    ...
    private Result existingResult(
        ...
        String oracleOutcome = stored.path("oracleOutcome")
                .asString(caseRun.securityOutcome());
        String reasonCode = stored.path("reasonCode")
                .asString(null);

        UUID oracleResultId = parseRequiredUuid(
                stored.path("oracleResultId").asString(null),
                "oracleResultId"
        );
        UUID findingId = parseOptionalUuid(
                stored.path("findingId").asString(null)
        );
        boolean deliveredToAgent =
                stored.path("deliveredToAgent").asBoolean();
    ...
    private boolean isReplayableTerminal(
    ...
    private ExecutionTarget requireTarget(
            UUID runId,
            UUID testCaseId
    ) {
        List<ExecutionTarget> targets =
                jdbcTemplate.query(
                        ...
                        select run.mode,
                               ...
                               test_case.payload_hash,
                               test_case.expected_invariant,
                               test_case.oracle_type,
                               test_case.preconditions_json::text
                          from test_runs run
                          join test_cases test_case
                            on test_case.suite_id = run.suite_id
                         where run.id = ?
                           and test_case.id = ?
                        """,
                        (resultSet, rowNumber) -> {
                            JsonNode preconditions =
                                    parseJson(
                                            resultSet.getString(
                                                    "preconditions_json"
                                            )
        ...
        if (targets.isEmpty()) {
            throw new BusinessException(
                    ...
                    "TestRun/TestCase pair not found"
            );
        }

        ExecutionTarget target = targets.getFirst();

        if (target.sandboxCaseKey() == null
                || target.currentApplicantId() == null) {
            throw new BusinessException(
                    ErrorCode.EVIDENCE_INCOMPLETE,
                    "FA-05 preconditions are incomplete"
            );
        }
    ...
    private void validateVariantAgainstTestCase(
            AttackVariant variant,
            ExecutionTarget target
    ) {
        if (!variant.category().equals(target.category())
                || !variant.severity().equals(
                target.severity()
        )
                || !variant.targetTool().equals(
                target.targetTool()
        )
                || !variant.invariantId().equals(
                target.expectedInvariant()
        )
                || !variant.oracleType().equals(
                target.oracleType()
        )) {
            throw new BusinessException(
                    ErrorCode.EVIDENCE_INCOMPLETE,
    ...
    private TestCaseRunStatus caseStatus(
    ...
    private boolean deliveredToAgent(
            AgentToolLoopService.ToolStep oracleStep
    ) {
        return oracleStep != null
                && oracleStep.delivery() != null
    ...
    private JsonNode parseJson(String value) {
        try {
            ...
            throw new BusinessException(
                    ErrorCode.EVIDENCE_INCOMPLETE,
                    "TestCase preconditions JSON is invalid"
            );
        }
    }
